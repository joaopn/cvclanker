...
const hiringcafeConfigSchema: SourceConfigSchema = {
  fields: [
    ...
    },
    {
      ...
      default: "",
      description:
        "Only request postings fetched in the last N days (maps to Hiring Cafe's dateFetchedPastNDays). That is the date Hiring Cafe indexed a posting, not the date it was published, so postings whose estimated publish date is older than this window are dropped during the scrape — its freshness is approximate either way. Setting a value also asks Hiring Cafe for newest-first results instead of its relevance ranking, so a capped run keeps the newest matches rather than the best-ranked ones. Leave blank to use the default 30-day window and relevance ranking.",
    },
  ],
  globalMappings: [
    {
